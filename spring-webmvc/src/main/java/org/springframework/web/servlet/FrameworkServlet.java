...
/**
 * Base servlet for Spring's web framework. Provides integration with a Spring
 * application context, in a JavaBean-based overall solution.
 *
 * <p>
 * This class offers the following functionality:
 * <ul>
 * <li>Manages a {@link org.springframework.web.context.WebApplicationContext
 * WebApplicationContext} instance per servlet. The servlet's configuration is
 * determined by beans in the servlet's namespace.
 * <li>Publishes events on request processing, whether or not a request is
 * successfully handled.
 * </ul>
 *
 * <p>
 * Subclasses must implement {@link #doService} to handle requests. Because this
 * extends {@link HttpServletBean} rather than HttpServlet directly, bean
 * properties are automatically mapped onto it. Subclasses can override
 * {@link #initFrameworkServlet()} for custom initialization.
 *
 * <p>
 * Detects a "contextClass" parameter at the servlet init-param level, falling
 * back to the default context class,
 ...
 * ApplicationContextInitializer} classes. The managed web application context
 * will be delegated to these initializers, allowing for additional programmatic
 * configuration, e.g. adding property sources or activating profiles against
 * the
 * {@linkplain org.springframework.context.ConfigurableApplicationContext#getEnvironment()
 * context's environment}. See also
 * {@link org.springframework.web.context.ContextLoader} which supports a
 * "contextInitializerClasses" context-param with identical semantics for the
 * "root" web application context.
 *
 * <p>
 * Passes a "contextConfigLocation" servlet init-param to the context instance,
 * parsing it into potentially multiple file paths which can be separated by any
 * number of commas and spaces, like "test-servlet.xml, myServlet.xml". If not
 * explicitly specified, the context implementation is supposed to build a
 ...
 * servlet-name "test" (leading to a "/WEB-INF/test-servlet.xml" default
 * location with XmlWebApplicationContext). The namespace can also be set
 * explicitly via the "namespace" servlet init-param.
 *
 * <p>
 ...
 * servlet instances. See {@link #FrameworkServlet(WebApplicationContext)}
 * Javadoc for details.
 *
 * @author Rod Johnson
...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	/**
	 * Comma-delimited ApplicationContextInitializer class names set through init
	 * param
	 */
	@Nullable
	 ...
	 * {@link ApplicationContextInitializer} classes should be used to further
	 ...
	 * registration of servlets is possible through the
	 ...
	 * {@linkplain ConfigurableApplicationContext#refresh() refreshed}. If it (a) is
	 ...
	 * the assumption that the user has performed these actions (or not) per his or
	 * her specific needs.
	 * <p>
	 ...
	 * retrieve the {@link WebApplicationContext} that this servlet is supposed to
	 * use.
	 */
	@Nullable
	...
	/**
	 * Specify a custom WebApplicationContext id, to be used as serialization id for
	...
	/**
	 * Set a custom namespace for this servlet, to be used for building a default
	...
	/**
	 * Return the namespace for this servlet, falling back to default scheme if no
	 * custom namespace was set: e.g. "test-servlet" for a servlet named "test".
	 */
	...
	/**
	 * Specify the set of fully-qualified {@link ApplicationContextInitializer}
	 ...
	 * it's good practice to let other application objects access the context this
	 * way.
	 */
	...
	/**
	 * Set whether this servlet should publish a ServletRequestHandledEvent at the
	 * end of each request. Default is "true"; can be turned off for a slight
	 * performance improvement, provided that no ApplicationListeners rely on such
	 * events.
	 * 
	 ...
	 * are spawned during request processing and only used for this request (that
	 * is, ending after their initial task, without reuse of the thread).
	 * <p>
	 * <b>WARNING:</b> Do not use inheritance for child threads if you are accessing
	 * a thread pool which is configured to potentially add new threads on demand
	 * (e.g. a JDK {@link java.util.concurrent.ThreadPoolExecutor}), since this will
	 * expose the inherited context to such a pooled thread.
	 */
	 ...
	 * all standard HTTP request methods as a response to the OPTIONS request). Note
	 * however that as of 4.3 the {@code DispatcherServlet} sets this property to
	 * "true" by default due to its built-in support for OPTIONS.
	 * <p>
	 * Turn this flag on if you prefer OPTIONS requests to go through the regular
	 * dispatching chain, just like other HTTP requests. This usually means that
	 * your controllers will receive those requests; make sure that those endpoints
	 * are actually able to handle an OPTIONS request.
	 * <p>
	 * Note that HttpServlet's default OPTIONS processing will be applied in any
	 * case if your controllers happen to not set the 'Allow' header (as required
	 * for an OPTIONS response).
	 */
	 ...
	 * <p>
	 * Note that HttpServlet's default TRACE processing will be applied in any case
	 * if your controllers happen to not generate a response of content type
	 ...
	 * 
	 * @return the WebApplicationContext for this servlet, or {@code null} if not
	 *         found
	 ...
	 * <p>
	 * Do not forget to register this servlet instance as application listener on
	 * the created context (for triggering its {@link #onRefresh callback}, and to
	 * call
	...
	/**
	 * Post-process the given WebApplicationContext before it is refreshed and
	 * activated as context for this servlet.
	 * <p>
	 * The default implementation is empty. {@code refresh()} will be called
	 * automatically after this method returns.
	 * <p>
	 * Note that this method is designed to allow subclasses to modify the
	 * application context, while {@link #initWebApplicationContext} is designed to
	 * allow end-users to modify the context through the use of
	 ...
	 * <p>
	 * See also {@link #postProcessWebApplicationContext}, which is designed to
	 * allow subclasses (as opposed to end-users) to modify the application context,
	 * and is called immediately before this method.
	 * 
	 * @param wac
	 *            the configured WebApplicationContext (not refreshed yet)
	 * @see #createWebApplicationContext
	 ...
	 * subclasses may override this method to perform any initialization they
	 * require.
	 * 
	...
	/**
	 * Refresh this servlet's application context, as well as the dependent state of
	 * the servlet.
	 * 
	 ...
	 * WebApplicationContext.
	 * <p>
	 * The default implementation calls {@link #onRefresh}, triggering a refresh of
	 * this servlet's context-dependent state.
	 * 
	 * @param event
	 ...
	 * {@code doHead}, with a {@code NoBodyResponse} that just captures the content
	 * length.
	 * 
	 * @see #doService
	 * @see #doHead
	 ...
	 * <p>
	 * Applies HttpServlet's standard OPTIONS processing otherwise, and also if
	 * there is still no 'Allow' header set after dispatching.
	 * 
	 * @see #doService
	 */
	@Override
	 ...
	 * <p>
	 * The actual event handling is performed by the abstract {@link #doService}
	 * template method.
	 */
	 ...
	 * holding a reference to the response), taking pre-bound attributes (and their
	 ...
	 * <p>
	 * The default implementation takes the name of the UserPrincipal, if any. Can
	 ...
	 * <p>
	 * This class intercepts calls to ensure that exception handling and event
